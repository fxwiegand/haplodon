# <p align="center"><img src="resources/haplodon.svg" alt="haplodon" style="width: 50%;"></p>



[![codecov](https://codecov.io/gh/fxwiegand/haplodon/graph/badge.svg?token=OSD3PSBS4L)](https://codecov.io/gh/fxwiegand/haplodon)
![Conda Version](https://img.shields.io/conda/v/bioconda/haplodon?label=bioconda)
![Conda Downloads](https://img.shields.io/conda/d/bioconda/haplodon?label=downloads)
...
To install **Haplodon**, you can install it via [Bioconda](https://bioconda.github.io/):

```bash
conda install -c bioconda haplodon
```
...
Use `haplodon --help` to view general help information, or `haplodon <command> --help` for specific command details.
...
```bash
haplodon build --calls path/to/calls.vcf --observations sample1=path/to/observations1.vcf sample2=path/to/observations2.vcf --min-prob-present 0.65 --output path/to/output/graphs.duckdb
```
...
- `--haplotype-metric <string>`: Metric to use for haplotype quantification. Valid values: `product`, `geometric-mean`, `minimum`. Defaults to `geometric-mean`.
- `--output <path>`: Path to the output file storing the calculated scores.

**Example:**

```bash
haplodon process --features path/to/features.gff --reference path/to/reference.fasta --graph path/to/graph.duckdb --output path/to/output/scores.duckdb
```
...
```bash
haplodon peptides --features path/to/features.gff --reference path/to/reference.fasta --graph path/to/graph.duckdb --interval 8-11 --sample sample1 --events event1 --events event2 --min-event-prob 0.8 --background-events event3 --max-background-event-prob 0.2 --output path/to/output/peptides.fasta
```
...
```bash
haplodon plot --input path/to/scores.duckdb --output scores.tsv
```

## Example

This is an example of using **Haplodon** to build a graph, process it, and output the results:

```bash
# Step 1: Build the variant graph
haplodon build --calls calls.vcf --observations sample1=observations1.vcf sample2=observations2.vcf --output graphs.duckdb

# Step 2: Process the graph with a GFF file
haplodon process --features features.gff --reference reference.fasta --graph graphs.duckdb --output scores.duckdb
...
**Haplodon** is licensed under the [MIT License](LICENSE).
